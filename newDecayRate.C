...
//______________________________________________________________________________
void decayRates::FFfunctionsBGL(double w, vector<parameter> FFpar, double& A1, double& A2, double& V){
...
double decayRates::dGdwDst(FFModel pFF, double VcbEff, double w){
  ...
  if(pFF.model=="CLN")
	...
	double A1{1}, A2{1}, V{1};
    FFfunctionsBGL(w,pFF.FFpars,A1,A2,V);
